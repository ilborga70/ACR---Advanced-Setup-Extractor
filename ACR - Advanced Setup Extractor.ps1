# ACR - Advanced Setup Extractor (Multi-Setup Splitter)
...
function Show-SetupSelectionDialog($setups) {
    ...
    if ($result -eq [System.Windows.Forms.DialogResult]::OK) {
        # L'operatore @() forza PowerShell a creare un array anche se c'è un solo elemento
        $selectedSetups = @()
        foreach ($idx in $checkList.CheckedIndices) {
            $selectedSetups += $setups[$idx]
        }
        return ,$selectedSetups # La virgola aiuta a preservare l'array nel return
    }
    return $null
}
...
function Export-SetupsToHtml($setups, $filePath) {
    ...
    # CSS per replicare fedelmente lo stile dell'immagine
    $sb.AppendLine("<!DOCTYPE html><html lang='it'><head><meta charset='UTF-8'>") | Out-Null
    ...
    $sb.AppendLine(".container { max-width: 950px; width: 100%; background: #fff; padding: 40px; box-sizing: border-box; box-shadow: 0 4px 15px rgba(0,0,0,0.1); }") | Out-Null
    ...
    $sb.AppendLine(".video-placeholder { width: 100%; aspect-ratio: 16/9; background: #000; margin-bottom: 30px; display: flex; align-items: center; justify-content: center; color: #555; font-weight: bold; border-radius: 4px; }") | Out-Null
    ...
    foreach ($setup in $setups) {
        # Funzione interna per pulire i valori e gestire le righe mancanti
        $getVal = { 
            param($pattern) 
            $line = $setup.Properties | Where-Object { $_ -match $pattern } | Select-Object -First 1
            if ($line -match ":") { return $line.Split(':', 2)[1].Trim() }
            # Gestione per i valori che appaiono nella riga successiva nel file TXT
        ...
        $sb.AppendLine("<div class='info-top'><b>Pilota:</b> $($setup.User) | <b>Vettura:</b> $($setup.Car) | <b>Tracciato:</b> $($setup.Track)</div>") | Out-Null
        $sb.AppendLine("<div class='video-placeholder'>[ INSERIRE QUI IL CODICE EMBED DEL VIDEO ]</div>") | Out-Null

        # --- SEZIONE 1: MECCANICA ---
        $sb.AppendLine("<h2>1. MECCANICA E TELAIO (CHASSIS & SUSPENSIONS)</h2>") | Out-Null
        $sb.AppendLine("<table><thead><tr><th>Parametro</th><th>Anteriore (FL/FR)</th><th>Posteriore (RL/RR)</th></tr></thead><tbody>") | Out-Null
        $sb.AppendLine("<tr><td>Pressione Pneumatici</td><td>$(& $getVal 'FrontLeft.TyrePressure') PSI</td><td>$(& $getVal 'RearLeft.TyrePressure') PSI</td></tr>") | Out-Null
        $sb.AppendLine("<tr><td>Campanatura (Camber)</td><td>$(& $getVal 'FrontLeft.Camber')°</td><td>$(& $getVal 'RearLeft.Camber')°</td></tr>") | Out-Null
        $sb.AppendLine("<tr><td>Convergenza (Toe)</td><td>$(& $getVal 'FrontLeft.Toe')</td><td>$(& $getVal 'RearLeft.Toe')</td></tr>") | Out-Null
        $sb.AppendLine("<tr><td>Rigidezza Molle</td><td>$(& $getVal 'FrontLeft.SpringStiffness') N/m</td><td>$(& $getVal 'RearLeft.SpringStiffness') N/m</td></tr>") | Out-Null
        $sb.AppendLine("<tr><td>Altezza (Adjuster Ring)</td><td>$(& $getVal 'FrontLeft.AdjusterRing') m</td><td>$(& $getVal 'RearLeft.AdjusterRing') m</td></tr>") | Out-Null
        $sb.AppendLine("<tr><td>Barra Antirollio (ARB)</td><td>$(& $getVal 'Axles.Front.ARB')</td><td>$(& $getVal 'Axles.Rear.ARB')</td></tr>") | Out-Null
        ...
        # --- SEZIONE 2: TRASMISSIONE ---
        $sb.AppendLine("<h2>2. TRASMISSIONE E CONTROLLI (DRIVETRAIN & ELECTRONICS)</h2>") | Out-Null
        $sb.AppendLine("<table><thead><tr><th>Sistema</th><th>Componente / Parametro</th><th>Valore / Specifica</th></tr></thead><tbody>") | Out-Null
        $sb.AppendLine("<tr><td>Cambio</td><td>Set Rapporti</td><td>$(& $getVal 'GearsSet')</td></tr>") | Out-Null
        $sb.AppendLine("<tr><td>Differenziale Centrale</td><td>Rapporto (Gears)</td><td>$(& $getVal 'CentreDifferentialRatio') - $(& $getVal 'CentreRatioToRear')</td></tr>") | Out-Null
        $sb.AppendLine("<tr><td>Differenziale Anteriore</td><td>Rampe / Preload</td><td>$(& $getVal 'Front.LSDRamps') - Preload: $(& $getVal 'Front.LSDPreload')</td></tr>") | Out-Null
        $sb.AppendLine("<tr><td>Differenziale Posteriore</td><td>Rampe / Preload</td><td>$(& $getVal 'Rear.LSDRamps') - Preload: $(& $getVal 'Rear.LSDPreload')</td></tr>") | Out-Null
        $sb.AppendLine("<tr><td>Differenziale Posteriore</td><td>Rapporto Finale</td><td>$(& $getVal 'Rear.DifferentialRatio')</td></tr>") | Out-Null
        $sb.AppendLine("<tr><td>Elettronica</td><td class='highlight'>ABS (Anti-lock Braking)</td><td class='highlight'>$(& $getVal 'Other.Electronic.ABS')</td></tr>") | Out-Null
        $sb.AppendLine("<tr><td>Elettronica</td><td class='highlight'>TCS (Traction Control)</td><td class='highlight'>$(& $getVal 'Other.Electronic.TCS')</td></tr>") | Out-Null
        $sb.AppendLine("</tbody></table>") | Out-Null

        # --- SEZIONE 3: IMPIANTO FRENANTE ---
        $sb.AppendLine("<h2>3. IMPIANTO FRENANTE (BRAKING SYSTEM)</h2>") | Out-Null
        $sb.AppendLine("<table><thead><tr><th>Componente</th><th>Anteriore</th><th>Posteriore</th></tr></thead><tbody>") | Out-Null
        $sb.AppendLine("<tr><td>Disco</td><td>$(& $getVal 'FrontLeft.Disc')</td><td>$(& $getVal 'RearLeft.Disc')</td></tr>") | Out-Null
        $sb.AppendLine("<tr><td>Pinza</td><td>$(& $getVal 'FrontLeft.Caliper')</td><td>$(& $getVal 'RearLeft.Caliper')</td></tr>") | Out-Null
        $sb.AppendLine("<tr><td>Pastiglie</td><td>$(& $getVal 'FrontLeft.PadCompound')</td><td>$(& $getVal 'RearLeft.PadCompound')</td></tr>") | Out-Null
        $sb.AppendLine("<tr><td>Cilindro Maestro</td><td>$(& $getVal 'MasterCylinderFront')</td><td>$(& $getVal 'MasterCylinderRear')</td></tr>") | Out-Null
        $sb.AppendLine("<tr><td>Ripartizione (Bias)</td><td colspan='2'>$(& $getVal 'FrontBias')</td></tr>") | Out-Null
        $sb.AppendLine("<tr><td>Freno a Mano</td><td colspan='2'>Forza: $(& $getVal 'HandbrakeForce')</td></tr>") | Out-Null
        $sb.AppendLine("</tbody></table>") | Out-Null

        $sb.AppendLine("<div class='footer-text'>Report generato sulla base dei dati tecnici ufficiali del pilota $($setup.User) - Assetto Corsa Rally</div>") | Out-Null
...
function Get-AllStringsFromFile($filePath, $textBoxOutput, $labelProgress) {
    ...
    try {
        ...
        $len = $bytes.Length
        $minStrLen = 2 # Abbassato a 2 per catturare numeri corti
        $maxStrLen = 200
        
        Update-Output "Scanning $len bytes for text data..." $textBoxOutput $labelProgress
        
        # Scansione byte per byte
        for ($i = 0; $i -lt $len - 4; $i++) {
            
            # Tentativo lettura lunghezza (Int32)
            $strLen = [System.BitConverter]::ToInt32($bytes, $i)
            
            # 1. Caso ASCII / UTF-8 (Lunghezza > 0)
            if ($strLen -gt $minStrLen -and $strLen -lt $maxStrLen) {
                ...
                if ($isValid) {
                    ...
                    # Filtro lasco: accettiamo tutto ciò che è stampabile
...
function Update-Output {
    ...
    $action = {
        ...
        $textBox.Refresh()
        if ($labelProgress) { $labelProgress.Refresh() }
    }
    if ($textBox.InvokeRequired) { $textBox.Invoke($action) } else { & $action }
}

# --- LOGICA ANALISI INTELLIGENTE (SPLITTER) ---
...
function Analyze-Data($filePath, $textBoxOutput, $labelProgress) {
    ...
    # 2. Identificazione Setup multipli basata sul pattern della Versione
    # Pattern: [CAR] -> [USER] -> [VERSION] -> [CAR] -> [TRACK]
    
    $versionIndices = @()
    # Regex per trovare la versione (es. 0.2.2.73152)
    $verRegex = "^\d+\.\d+\.\d+\.\d+$"
    ...
    if ($versionIndices.Count -eq 0) {
        ...
        # Fallback: restituisce tutto come un unico setup ignoto
    ...
    # 3. Costruzione Oggetti Setup
    for ($k = 0; $k -lt $versionIndices.Count; $k++) {
        $vIdx = $versionIndices[$k]
        
        # Identifica Header relativo alla versione trovata
        # vIdx-2: CarModel
        # vIdx-1: Username (Setup Name)
        # vIdx+0: Version
        # vIdx+1: CarModel (Ripetuto)
        # vIdx+2: Track
        ...
        # Definisci l'intervallo delle proprietà
        $startPropIdx = $vIdx + 3
        
        # La fine è l'inizio del prossimo setup (meno l'header del prossimo) oppure la fine del file
    ...
    if ($script:results) {
        ...
        if ($selectedSetups -and $selectedSetups.Count -gt 0) {
            ...
            if ($d.ShowDialog() -eq [System.Windows.Forms.DialogResult]::OK) {
                ...
                # Esportazione HTML (Passa sempre l'array)
                Export-SetupsToHtml $selectedSetups $htmlPath
                
                [System.Windows.Forms.MessageBox]::Show("Export Complete!`nSaved: $basePath e $htmlPath")
            }
        } else {
